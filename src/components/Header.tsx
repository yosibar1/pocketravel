  ...
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/90 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-4 px-4">
        <Link href="/" className="flex items-center gap-2 text-xl font-extrabold text-indigo-950">
          <span aria-hidden>✈️</span>
          <span>
            Pocket
            <span className="bg-gradient-to-l from-fuchsia-600 to-rose-500 bg-clip-text text-transparent">
              Ravel
            </span>
          </span>
        </Link>
        ...
        <div className="flex items-center gap-2">
          ...
          {userName ? (
            ...
            <form action="/api/auth/signin/google" method="post">
              <button
                type="submit"
                className="rounded-full bg-gradient-to-l from-fuchsia-600 to-rose-500 px-4 py-1.5 text-sm font-semibold text-white transition-transform hover:scale-105"
              >
                {t.signIn}
              </button>
            </form>
        ...
        {links.map((link) => (
          <Link
            ...
            className={`rounded-full px-3 py-1.5 text-xs font-semibold ${
              isActive(link.href) ? "bg-indigo-950 text-white" : "text-slate-600"
            }`}
          >
            {link.label}
          </Link>
